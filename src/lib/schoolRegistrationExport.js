// 학교 화학물질 등록 Excel — 시약목록 선택목록(→ Excel 내보내기 → 학교 화학물질 등록 양식)의 핵심 로직.
// 자료 탭에 있던 SchoolRegistrationView(최근 입고 전체를 훑어 고르는 편집 그리드)는 시약목록과
// 중복되는 업무 도구라 제거됐다(2026-09-22, 자료 단순화 Phase) — 행 만들기(그룹핑 없음, Lot 1건 =
// 행 1건) · CAS 매칭 · XLSX 생성 로직은 그대로 여기 남아 선택목록 경로에서만 쓰인다.
...
// 다를 수 있어 자동으로 채우지 않고 사용자 확인을 받는다.
...
export function writeSchoolRegistrationExcel(includedRows) {
  const aoa = [
    ...
    // volume/quantity가 숫자 문자열이면 Number() 변환하고, 검증을 안 거친 빈 값은 셀도 비워둔다(0으로 안 씀).
    ...includedRows.map(r => [
...
// 선택목록(시약목록) 전용 진입점 — 편집 그리드 없이 바로 생성한다(큰 wizard를 만들지 않음). 그 대신:
