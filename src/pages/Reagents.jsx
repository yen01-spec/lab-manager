...
function Reagents() {
  ...
  return (
    <div>
      <h1 style={{ color: '#1e3a5f', marginBottom: '24px' }}>?úÏïΩ Í¥ÄÎ¶?/h1>
      ...
      {searchResults.length > 0 && (
        <div style={{ marginBottom: '32px' }}>
          <h2 style={{ color: '#1e3a5f' }}>Í≤Ä??Í≤∞Í≥º ({searchResults.length}Í∞?</h2>
      ...
      {searchResults.length === 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: '24px' }}>
          <div>
            <h2 style={{ color: '#1e3a5f', marginBottom: '12px' }}>?ÑÏπò</h2>
            {rooms.map(room => (
  ...
  return (
    <div onClick={onClick} style={{
      ...
      <div style={{ fontSize: '13px' }}>
        ÎØ∏Í∞úÎ¥?<strong>{totalSealed}Î≥?/strong>
        {isLow && <span style={{ color: '#e53e3e', marginLeft: '8px' }}>?¨Í≥† Î∂ÄÏ°?/span>}
      </div>
    </div>
  )
}
