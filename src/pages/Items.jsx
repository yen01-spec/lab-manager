            ...
            {rooms.map(room => (
              <div key={room} style={{ marginBottom: '8px' }}>
                ...
                {locations.filter(l => l.room === room).map(loc => (
                  <div
                    key={loc.id}
                    onClick={() => { setSelectedLocation(loc); fetchItemsByLocation(loc.id) }}
                    style={{
...
function ItemCard({ item, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      <div style={{ color: '#666', fontSize: '13px', marginBottom: '8px' }}>{item.category || '-'}</div>
      <div style={{ fontSize: '13px' }}>
        재고 <strong>{totalSealed}</strong>
