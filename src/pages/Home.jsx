      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
        ...
        <div style={{ border: '1px solid #e2e8f0', borderRadius: '8px', padding: '16px' }}>
          ...
          {notices.length === 0 ? (
            ...
            notices.map(n => (
              <div key={n.id} style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '12px', marginBottom: '12px' }}>
                <div style={{ fontWeight: 'bold' }}>{n.title}</div>
                <div style={{ color: '#666', fontSize: '14px' }}>{n.content}</div>
                <div style={{ color: '#999', fontSize: '12px' }}>{new Date(n.created_at).toLocaleDateString()}</div>
              </div>
            ))
          )}
        </div>
          ...
          {expiringSoon.length === 0 ? (
            ...
            expiringSoon.map(e => (
              <div key={e.id} style={{ marginBottom: '8px' }}>
                ...
                <span style={{ color: '#b7791f', marginLeft: '8px' }}>{e.expiry_date}</span>
              </div>
            ))
          )}
        </div>
