...
- ~~**알려진 후속 과제**: staging 전용 QA 스크립트(`scripts/qa/qa-e-ui.mjs`, `qa-f-network.mjs`)는 아직 `/reagents/bulk-edit`를 직접 열도록 되어 있어...~~ → **2026-09-22 QA debt closure Phase에서 해결됨**(아래 "최종 staging QA debt closure" 참고).
...
- ~~**알려진 후속 과제**: `scripts/qa/qa-e-ui.mjs`/`qa-f-network.mjs`는 이전 Phase부터의 QA debt...~~ → **2026-09-22 QA debt closure Phase에서 해결됨**(아래 "최종 staging QA debt closure" 참고). 그 Phase에서 article-level 재정렬(article reorder) 실브라우저 검증도 처음으로 추가됨(이 Phase는 탭 재정렬만 다룸).

## 최종 staging QA debt closure — 구 QA 스크립트 최신화 + 통합 IA 실브라우저 검증 (2026-09-22, frontend + QA script 전용, migration/RLS/RPC 변경 없음)
- **목적**: 이전 두 Phase가 스스로 flag한 QA debt(`qa-e-ui.mjs`/`qa-f-network.mjs`가 옛 `/reagents/bulk-edit` 페이지·옛 공지/자료 구조를 직접 테스트)를 정리하고, 통합 시약목록 액션허브 + 새 자료실 CMS를 실제 staging 브라우저로 처음부터 다시 검증. article 재정렬(article reorder)은 이전 Phase 보고에서 누락이 명시적으로 지적된 항목이라 이번에 처음 실브라우저로 검증.
- **`qa-e-ui.mjs` 재작성**: STEP21(옛 `/reagents/bulk-edit` BulkEditTab 테스트)을 `/reagents/list` 통합 액션허브 테스트로 전면 교체(선택→액션바, 위치이동/폐기 2단계 Lot 선택, 같은 Lot No. 독립, 묶음 행 가드, 구매요청 prefill, 정보수정 단일/큐, Excel, A–Z, 일괄검색, 모바일 bottom sheet). STEP22 breadcrumb 표에서 이제 redirect-only인 라우트(옛 `/reagents/bulk-edit`/`/notices`/`/notices/:id`/`/safety`/`/safety/:id`)를 빼고 새 `STEP-REDIRECTS`(5개 라우트, replace 시맨틱 — back 한 번에 죽은 경로를 거치지 않고 바로 이전 페이지로 감)로 분리. 새 `STEP-RESOURCES`/`STEP-RESOURCES-admin`(canonical `/resources` 구조·필드·부재 확인·관리자 컨트롤)도 추가.
- **`qa-f-network.mjs` 재작성**: 옛 BulkEdit reload 검증(별도 페이지 상태 가정)을 `/reagents/bulk-edit → /reagents/list` redirect 확인 한 줄로 교체. 새 `STEP28`/`STEP28-admin`으로 자료실 네트워크(resource_tabs/resource_articles/resource_files 배치 조회, notices 조회 0, 탭 전환/관리자 모달이 전체 재조회를 유발하지 않음)를 실측 확인.
- **실브라우저 QA로 발견·수정한 진짜 버그 2건**:
  1. **`resource_files` N+1**: 첨부파일 없는 글의 `filesByArticle.get(id) || []`가 매 렌더 새 배열을 만들어 `ResourceArticleFiles`의 `initialRows` identity가 흔들렸고, dev StrictMode의 mount 이중 호출과 겹쳐 글 30개 로드 시마다 개별 `resource_files` 요청이 최대 85건까지 발생했다(자료실 방문 1회 = 정상 2건이어야 함). `Resources.jsx`에 모듈 레벨 안정 참조(`EMPTY_FILES`) 도입 + `ResourceArticleFiles.jsx`를 `useState` lazy initializer(초기 데이터, StrictMode에도 정확히 1회만 호출됨) + 항상 새로 조회하는 별도 `refresh()`(추가/삭제 후, "다시 시도")로 재설계. STEP28 네트워크 감사: 32→2건(최초), 17→0건(탭 전환), 85→0건(관리자 탭 관리 모달).
  2. **글 순서(article `sort_order`) 미지정 버그**: `ResourceArticleEditor`가 새 글에 `sortOrder`를 넘기지 않아 항상 0으로 저장됐다 — 같은 탭에 글이 이미 있으면 전부 sort_order=0으로 겹쳐 화면 순서가 작성 순서가 아니라 DB가 동률을 반환하는 임의 순서가 됐다(STEP-M article reorder 테스트로 발견: A,B 순서로 작성했는데 B,A로 렌더). `lib/resources.js`의 `createResourceArticle`이 `createResourceTab`과 같은 패턴(같은 탭 안 최대 sort_order+1)으로 끝에 붙이도록 수정.
  3. **(참고, 수정 안 함) dev 전용 콘솔 노이즈**: A–Z 점프(`react-virtual`의 `scrollToIndex`)가 `@vite/client`(Vite HMR) 경로에서 "flushSync was called from inside a lifecycle method" 콘솔 에러를 낸다 — 원본 위치가 앱 코드가 아니라 `@vite/client`이고 StrictMode 이중 effect와 같은 카테고리의 dev-only 증상(production build엔 없음)이라 QA 스크립트에서 이 특정 문구만 필터링해 노이즈로 기록(`note()`), 앱 코드는 손대지 않음.
- **`qa-resource-library.mjs`에 article reorder 테스트 추가(Phase M)**: 새 QA 탭에 글 A,B를 순서대로 작성 → B에서 "위로 이동" → 화면이 B,A로 바뀜 → 하드 새로고침 후에도 B,A 유지(=서버 저장, 화면 state 아님) → anon REST로 `resource_articles.sort_order`를 직접 읽기전용 조회해 DB에서도 B<A 확인 → "아래로 이동"으로 A,B 복원 → QA 글/탭 정리. 25/25 PASS(기존 CRUD 14건 포함, 전부 재확인).
- **`scripts/qa/lib.mjs`의 `shot()`을 non-fatal로 변경**: 폰트 로딩 타임아웃 등 스크린샷 자체의 환경 플레이크가 스위트 전체를 죽이지 않도록(스크린샷은 증거 첨부일 뿐 assertion이 아님) try/catch + `note()`로 기록.
- **재실행 결과(모두 staging 실데이터/실브라우저, production 접속 0)**: `qa-e-ui.mjs` 125/125, `qa-f-network.mjs` 33/33, `qa-resource-library.mjs`(article reorder 포함) 25/25, `qa-resource-visual.mjs` 19/19(회귀 재확인), `qa-resource-rls.mjs` 8/8(회귀 재확인).
- **Dead-code/stale-reference 감사**: `scripts/qa`·`scripts/ui`·`scripts/staging`·`docs` 전수 grep — `/reagents/bulk-edit`/`/notices`/`/safety` 남은 참조는 전부 의도된 redirect 테스트 문자열이거나(`test-reagent-ux-consolidation.mjs`, `test-resources.mjs`, 새 `qa-e-ui.mjs`/`qa-f-network.mjs`) 정확한 설명 주석(`test-admin-gate.mjs`, `test-reagent-search-input.mjs`, `audit-a11y-mobile.mjs`) — 갱신이 필요한 stale active flow는 발견되지 않음. `notices`/`notice_files` 테이블 참조(`test-backup-restore-full.mjs`)는 백업/복원 기능이 그 테이블 데이터를 여전히 보존하므로 정상. `scripts/staging/inspect-resources-pre.mjs`(마이그레이션 전 1회성 조사 도구)는 더는 필요하지 않지만 read-only라 위험 없음 — 유지(참고용, 삭제 안 함).
- **범위 밖으로 확정한 것**: production migration/RLS/RPC 변경 없음(이 Phase는 frontend 코드 + QA 스크립트만). production pending migration 개수는 이번 Phase에서 재확인하지 않음(다음 Production Admin Gate Phase에서 재계산).
