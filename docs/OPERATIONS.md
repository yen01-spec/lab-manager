...
- **백업/복원**: 관리자 > 백업/복원. 백업 = JSON ZIP(복원용: manifest{backup_version, created_at, app_version, schema_version, table_counts, table_digests, 파일 sha256} + tables/*.json) + Excel(조회용). 범위: students, locations, reagents, reagent_lots, 위치/요청/재고 이력, inventory_*, admin_logs. 제외: Auth 사용자, admin_users, fcm_tokens, student_sessions, app_settings, Storage 파일, 구매요청/공지/레거시 items. 복원 = 빈 대상에만(병합 복원 없음), 서버 RPC `admin_restore_full` 한 트랜잭션: manifest/스키마 → 빈 대상 → PK 중복 → FK 고아 → 삽입(제약 위반 시 전체 롤백) → 행 수/digest 검증 → 시퀀스 보정. dry-run 은 항상 롤백(쓰기 0). 실행은 app_settings.restore_enabled='true' + p_confirm='RESTORE' 필요(기본 비활성 = production). 복원 전 현재 DB 백업 내려받기를 UI 가 강제.
